...
import { useAsyncData, usePaginatedData } from '../hooks/useAsyncData';
import { useToast, useSuccessToast, useErrorToast } from '../contexts/ToastContext';
import { useResponsive, ResponsiveGrid, ResponsiveContainer } from '../contexts/ResponsiveContext';
...
const fetchFlightData = async (page: number, pageSize: number) => {
  await new Promise(resolve => setTimeout(resolve, 800));
  const allFlights = Array.from({ length: 100 }, (_, i) => ({
    id: `VS${String(i + 1).padStart(3, '0')}`,
    origin: ['LHR', 'MAN', 'LGW'][i % 3],
    destination: ['JFK', 'LAX', 'BOS', 'SFO'][i % 4],
    status: ['On Time', 'Delayed', 'Boarding'][i % 3],
    departure: new Date(Date.now() + (i * 3600000)).toLocaleTimeString(),
    delay: Math.random() > 0.7 ? Math.floor(Math.random() * 60) : 0
  }));
  ...
  return {
    data: allFlights.slice(start, end),
...
const EnhancedFlightDashboard: React.FC = () => {
  ...
  // Async data with error handling and loading states
  const flightSummary = useAsyncData(fetchFlightSummary, {
    dependencies: [refreshKey],
    onSuccess: () => successToast('Flight summary updated'),
    onError: (error) => errorToast(`Failed to load flight summary: ${error}`)
  });

  const paginatedFlights = usePaginatedData(fetchFlightData, {
    pageSize: isMobile ? 5 : isTablet ? 8 : 10,
    dependencies: [refreshKey]
  });

  const weatherData = useAsyncData(fetchWeatherData, {
    dependencies: [refreshKey],
    ...
    <ResponsiveContainer maxWidth="full" className="py-6">
      <div className="space-y-6">
        ...
        <ComponentErrorBoundary>
          <LoadingState loading={flightSummary.loading} fallback={
            ...
            {flightSummary.data && (
              <ResponsiveGrid cols={{ sm: 2, md: 3, lg: 5 }}>
                <StatCard
                  ...
                  title="Total Flights"
                  value={flightSummary.data.totalFlights}
                  color="blue"
                />
                <StatCard
                  icon={<TrendingUp className="h-6 w-6" />}
                  title="On Time"
                  value={flightSummary.data.onTime}
                  color="green"
                />
                <StatCard
                ...
                <StatCard
                  ...
                  title="Avg Delay"
                  value={`${flightSummary.data.avgDelay}m`}
                  color="purple"
                />
              </ResponsiveGrid>
            )}
        ...
        <ComponentErrorBoundary>
          <div className="bg-white dark:bg-slate-800 rounded-lg p-6">
            ...
            <LoadingState loading={paginatedFlights.loading} fallback={
                  ...
                  {paginatedFlights.data.map((flight) => (
                    <div key={flight.id} className="flex justify-between items-center p-3 border rounded hover:bg-gray-50 dark:hover:bg-slate-700">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        {flight.delay > 0 && (
                          <span className="text-xs text-orange-600">+{flight.delay}m</span>
                        )}
                      </div>
                    </div>
                  ))}
            ...
            {paginatedFlights.totalPages > 1 && (
              <div className="flex justify-between items-center mt-6 pt-4 border-t">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={paginatedFlights.previousPage}
                    disabled={!paginatedFlights.hasPreviousPage}
                    className="px-3 py-1 text-sm border rounded disabled:opacity-50"
                  >
                    Previous
                  </button>
                  <button
                    onClick={paginatedFlights.nextPage}
                    disabled={!paginatedFlights.hasNextPage}
                    className="px-3 py-1 text-sm border rounded disabled:opacity-50"
                  >
                    Next
                  </button>
...
const StatCard: React.FC<StatCardProps> = ({ icon, title, value, color }) => {
  const colorClasses = {
      blue: 'text-va-blue bg-va-blue/10',
      green: 'text-va-green bg-va-green/10',
      orange: 'text-va-amber bg-va-amber/10',
      red: 'text-va-red bg-va-red/10',
      purple: 'text-va-purple bg-va-purple/10'
  };
...
export default EnhancedFlightDashboard;
