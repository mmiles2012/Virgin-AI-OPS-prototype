  ...
  useEffect(() => {
    ...
    const testAviationStack = async () => {
      ...
      try {
        ...
        if (resultDiv) {
          resultDiv.innerHTML = `
            <div class="p-4 rounded ${result.success ? 'bg-green-900/20 border border-green-600 text-green-300' : 'bg-red-900/20 border border-red-600 text-red-300'}">
              <div class="font-medium">AviationStack</div>
              <div class="text-sm mt-1">${result.message}</div>
              ${result.data ? `<pre class="text-xs mt-2 p-2 bg-gray-800 rounded overflow-x-auto">${JSON.stringify(result.data, null, 2)}</pre>` : ''}
            </div>
          `;
        }
      } catch (error) {
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 mb-6">
          <button
            ...
            className="w-full px-4 py-3 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors cursor-pointer"
          >
          ...
          <button
            id="test-opensky"
            className="w-full px-4 py-3 bg-green-600 text-white rounded hover:bg-green-700 transition-colors cursor-pointer"
          >
          ...
          <button
            id="test-mapbox"
            className="w-full px-4 py-3 bg-purple-600 text-white rounded hover:bg-purple-700 transition-colors cursor-pointer"
          >
            Test Mapbox API
          </button>
        </div>
