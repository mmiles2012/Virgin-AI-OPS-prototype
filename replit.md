...
- July 04, 2025. **Technical Accuracy Corrections**: Fixed Boeing 787-9 engine specifications from generic "GEnx-1B / Trent 1000" to accurate "Rolls-Royce Trent 1000" for Virgin Atlantic fleet; enhanced geographic routing with region-specific airport selections including Indian subcontinent airports (VABB Mumbai, VIDP Delhi, VOBL Bangalore) for authentic LHR-BOM route diversions; integrated intelligent airport selection based on route analysis to prevent unrealistic diversion recommendations; scenario generator now uses same enhanced diversion analysis engine as what-if system ensuring coordinated, geographically accurate recommendations
